...
class Installer:
    ...
    def log(self, *args, level=LOG_LEVELS.Debug, file=None, **kwargs):
        if not file:
            if "logfile" not in storage:
                ...
                storage[
                    "logfile"
                ] = f"{log_root}/install-session_{self.init_time}.{self.milliseconds}.log"
    ...
    def __exit__(self, *args, **kwargs):
        ...
        if not (missing_steps := self.post_install_check()):
            ...
            self.log(
                f"Detailed error logs can be found at: {log_path}",
                level=LOG_LEVELS.Warning,
            )
            self.log(
                f"Submit this zip file as an issue to https://github.com/Torxed/archinstall/issues",
                level=LOG_LEVELS.Warning,
            )
            return False
    ...
    def pacstrap(self, *packages, **kwargs):
        ...
        if (sync_mirrors := sys_command("/usr/bin/pacman -Syy")).exit_code == 0:
            ...
            self.log(
                f"Could not sync mirrors: {sync_mirrors.exit_code}",
                level=LOG_LEVELS.Info,
            )
    ...
    def configure_nic(
        ...
        with open(
            f"{self.mountpoint}/etc/systemd/network/10-{nic}.network", "a"
        ) as netconf:
    ...
    def add_bootloader(self, bootloader="systemd-bootctl"):
        self.log(
            f"Adding bootloader {bootloader} to {self.boot_partition}",
            level=LOG_LEVELS.Info,
        )
