...
class Partition():
	...
	def format(self, filesystem):
		...
		if filesystem == 'btrfs':
			...
			if (handle := sys_command(f'/usr/bin/mkfs.xfs -fs {self.path}')).exit_code != 0:
				raise DiskError(f'Could not format {self.path} with {filesystem} because: {b"".join(handle)}')
			self.filesystem='xfs'
		elif filesystem== 'f2fs':
			if (handle := sys_command(f'/usr/bin/mkfs.f2fs -fs {self.path}')).exit_code != 0:
				raise DiskError(f'Could not format {self.path} with {filesystem} because: {b"".join(handle)}')
	...
	def __enter__(self, *args, **kwargs):
		...
		if self.mode == MBR:
			if sys_command(f"/usr/bin/parted -s {self.blockdevice.device} mklabel  msdos").exit_code==0:
				...
				raise DiskError(f'Problem setting the partition format to MBR:', f'/usr/bin/parted -s {self.blockdevice.device} mklabel msdos')
		else:
