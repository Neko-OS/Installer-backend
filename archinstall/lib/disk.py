...
class Partition:
    ...
    def format(self, filesystem):
        log(
            f"Formatting {self} -> {filesystem}",
            level=LOG_LEVELS.Info,
            file=storage.get("logfile", None),
        )
        if filesystem == "btrfs":
            ...
            if (
                handle := sys_command(f"/usr/bin/mkfs.xfs -fs {self.path}")
            ).exit_code != 0:
                raise DiskError(
                    f'Could not format {self.path} with {filesystem} because: {b"".join(handle)}'
                )
            ...
            if (
                handle := sys_command(f"/usr/bin/mkfs.f2fs -fs {self.path}")
            ).exit_code != 0:
                raise DiskError(
                    f'Could not format {self.path} with {filesystem} because: {b"".join(handle)}'
                )
...
class Filesystem:
    ...
    def __enter__(self, *args, **kwargs):
        ...
        if self.mode == MBR:
            if (
                ...
                raise DiskError(
                    f"Problem setting the partition format to MBR:",
                    f"/usr/bin/parted -s {self.blockdevice.device} mklabel msdos",
                )
        else:
    ...
    def add_partition(self, type, start, end, format=None):
        log(
            f"Adding partition to {self.blockdevice}",
            level=LOG_LEVELS.Info,
            file=storage.get("logfile", None),
        )
